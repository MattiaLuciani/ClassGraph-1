...
def lp1(max_iteration, data):
    ...
    for v in range(max_iteration): #for i = 1,2,...,#iter do
        ...
        for i in range(len(data)):
            ...
            len_line = len(data[i])
            if len_line > 3: #if idj received the information (lid1,p1),..(lidk,pk) #cosi prendo i nodi non etichettati da etichettare con i nodi vicino [49946, 0, [[49734, 0.94]], [1925779, 0.71], [1327964, 0.67]]
                possible_labels = []
                ...
                for j in range(len(possible_labels)):
                    if len(summing_list) == 0:
                        ...
                        #qui in poi bisogna cambiare gli indici per fargli prendere l'etichetta invece del peso 
                    else:
                        summing_list[len(summing_list) - 1][1] = summing_list[len(summing_list) - 1][1] + possible_labels[j][1]
                        #possible_labels[j][0] peso
